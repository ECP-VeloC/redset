...
#if ENABLE_CUDA
__global__ void xor_gpu(unsigned char* a, unsigned char* b, int n)
{
  ...
  const redset_base* d)
{
  ...
  unsigned char* recv_buf_dev;
  cudaMalloc(&send_buf_dev, redset_mpi_buf_size);
  cudaMalloc(&recv_buf_dev, redset_mpi_buf_size);

  /* switch send/recv to use device buffers */
  send_buf = send_buf_dev;
  recv_buf = recv_buf_dev;
#endif
